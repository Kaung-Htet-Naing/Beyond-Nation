...
TlsCanvasFont.prototype.getGlyph = function (uChar) {
  return this.font.getGlyph(uChar);
};
TlsCanvasFont.prototype.nodeFontSize = function (node) {
  return this.font.nodeFontSize(node);
};
TlsCanvasFont.prototype.computedStyle = function (node) {
  return this.font.computedStyle(node);
};
 ...
 */
TlsCanvasFont.prototype.appendText = function (
  parent,
  fontSize,
  text,
  color,
  ...
  try {
    ...
    } else ctx.fillStyle = foreColor.asRgb();
  } catch (e) {
    TlsDebug.print("Color exception" + color + " " + e);
    color = oldColor;
  }
...
TlsCanvasFont.prototype.drawTextRun = function (ctx, run) {
  ...
  try {
    ...
    TlsDebug().print(e + " " + e.description);
  }
  ctx.restore();
  return run;
};
...
TlsCanvasSvgPath.prototype.M = function (args) {
  ...
  TlsDebug().print(
    "ctx.moveTo (" + (this.dx + this.x) + "," + (this.dy + this.y) + ")",
  );
  while (args.length > 0) {
    this.x = Number(args.shift());
    this.y = Number(args.shift());
...
TlsCanvasSvgPath.prototype.H = function (args) {
  while (args.length > 0) {
    ...
    this.ctx.lineTo(this.dx + this.x, this.dy + this.y);
    TlsDebug().print(
      "ctx.lineTo(" + (this.dx + this.x) + "," + (this.dy + this.y) + ")",
    );
  }
  this.prevCx = this.x;
  this.prevCy = this.y;
