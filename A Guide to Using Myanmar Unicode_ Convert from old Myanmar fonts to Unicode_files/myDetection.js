...
function myUnsupportedMsg(myIconPath) {
  return (
    ...
    "<p class='myTextMsg'><img src='" +
    myIconPath +
    "myNoUnicode0.png' /><br />" +
    "<a href='http://www.thanlwinsoft.org/ThanLwinSoft/Downloads/#fonts'><img src='" +
    myIconPath +
    "myNoUnicode1.png' /></a></p>" +
...
function mySupportedMsg(myIconPath) {
  return (
    "<p class='myGood'>Congratulations your browser is Myanmar Unicode enabled! <br />" +
...
var myCommon = {
  createElement: function (eName) {
    if (document.createElementNS)
      return document.createElementNS("http://www.w3.org/1999/xhtml", eName);
...
MyNodeParser.prototype.parse = function () {
  var node = this.node;
  if (node == undefined || node.tagName == undefined || node.nodeType != 1)
    return;
  myUnicode.nodeCount++;
  ...
  if (node.tagName.toLowerCase() == "input") {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      if (child.nodeType == 3) {
        ...
        if (
          myCommon.getId(child) != "myDebug" &&
          node.getAttribute("class") != "tlsNoConvert"
        )
...
function TlsMyUnicode() {
  ...
  this.debug = function () {
    return document.getElementById("myDebug");
  };
  return this;
}
...
TlsMyUnicode.prototype.checkWithMsg = function () {
  if (this.check()) {
    ...
    if (myUnsupported.length > 0) document.writeln(myUnsupported);
  }
};
...
TlsMyUnicode.prototype.check = function () {
  ...
  if (myW1 && myW2) {
    ...
    if (
      (myW1Width == undefined && myW2Width == undefined) ||
      (myW2Width == 0 && myW1Width == 0)
    ) {
  ...
  if (widthTest != 0) document.body.removeChild(widthTest);
...
TlsMyUnicode.prototype.initParse = function (theImgPrefix) {
  ...
  else this.addScript(theImgPrefix + "canvas/tlsCanvasFont.js");
...
TlsMyUnicode.prototype.createNotice = function () {
  ...
  if (document.createElementNS)
    notice = document.createElementNS("http://www.w3.org/1999/xhtml", "div");
  else notice = document.createElement("div");
  notice.setAttribute("id", "myParseNotice");
  notice.setAttribute(
    "title",
    "Your browser doesn't support Myanmar Unicode or you don't have a suitable font. Please see www.thanlwinsoft.org.",
  );
  // firefox supports position: fixed, so place it at the bottom of the screen
  notice.setAttribute(
    "style",
    "background-color: red; color: white; position: fixed; right: 0px; bottom: 0px; z-index: 5;",
  );
  if (notice.style) {
  ...
  var br;
  if (document.createElementNS)
    br = document.createElementNS("http://www.w3.org/1999/xhtml", "br");
  ...
  if (this.queue.length > 0) {
    ...
    if (this.conversionCount > this.countThreshold || this.parseCount > 100) {
      ...
      if (notice.childNodes.length > 80)
        for (
          var i = notice.childNodes.length - 1;
          i > this.noticeNodeOffset;
          i--
        )
          notice.removeChild(notice.childNodes[i]);
      notice.appendChild(document.createTextNode("."));
    } else this.parseNextNode();
  } else {
    this.conversionCount = 0;
    this.parseCount = 0;
    this.hideNotice();
...
TlsMyUnicode.prototype.queueNode = function (nodeParser) {
  ...
  if (this.queue.length == 1) this.parseNextNode();
};
...
TlsMyUnicode.prototype.inRange = function (code) {
  ...
  if (code >= this.codeStart && code < this.codeEnd) return true;
  return false;
};
  ...
  for (var i = 0; i < text.length; i++) {
    ...
    if (this.inRange(text.charCodeAt(i))) {
      if (typeof docFrag == "undefined") {
        ...
        if (
          typeof tlsFontCache != "undefined" &&
          tlsFontCache.hasFont(this.fontData)
        ) {
          if (!this.canvasFont) {
      ...
      for (j = i + 1; j < i + text.length; j++) {
        ...
        if (
          typeof myParser != "undefined" &&
          myParser.canBreakAfter(text, j - 1)
        ) {
          break;
        }
      }
      try {
        ...
        if (typeof this.canvasFont != "undefined") {
          ...
          TlsDebug().print(
            "fontsize:" + fontSize + " computed:" + computedStyle.fontSize,
          );
          if (
            this.canvasFont.appendText(
              docFrag,
              fontSize,
              text.substring(i, j),
              textColor,
        ...
        if (typeof TlsDebug != "undefined")
          TlsDebug().print(
            "Exception:" + e + (e.description ? e.description + e.line : ""),
          );
      }
