...
function TlsMyanmarConverter(data) {
  ...
  this.unicodeSequence = new Array(
    "kinzi",
    null,
    "lig",
    null,
...
TlsMyanmarConverter.prototype.buildRegExp = function (sequence, isUnicode) {
  ...
  for (var i = 0; i < sequence.length; i++) {
      ...
      if (this.data[sequence[i]][j] && this.data[sequence[i]][j].length > 0) {
        ...
        if (isUnicode) {
          ...
          if (underscore == -1) {
            ...
            this.reverse[sequence[i]][this.data[sequence[i]][j]] = j.substring(
              0,
              underscore,
            );
          }
        } else {
          var escapedAlternate = this.data[sequence[i]][j].replace(
            escapeRe,
            "\\$1",
          );
    ...
    if (sequence[i] == "cons") {
      ...
      if (isUnicode) pattern += "?";
      else pattern += "*"; // these are frequently multi-typed
    } else {
      pattern += "?";
    }
  }
 ...
 * @internal
 */
TlsMyanmarConverter.prototype.toUnicodeMapper = function (
  inputText,
  matchData,
  prevSyllable,
) {
    ...
    if (syllable[component])
      this.debug.print(
        "Unicode Syllable:" +
          ...
          syllable[component] +
          " / " +
          this.reverse[component][matchData[g]],
      );
      ...
      if (component == "yapin") {
        if (syllable[component].charAt(1) == "ွ") {
          ...
          if (syllable[component].length > 2) {
            if (syllable[component].charAt(2) == "ှ") syllable["hatoh"] = "ှ";
            else {
              this.debug.print(
                "Unhandled yapin ligature: " + syllable[component],
              );
            }
          }
          syllable[component] = syllable[component].substring(0, 1);
        } else if (
          syllable[component].charAt(1) == "ှ" ||
          syllable[component].length > 2
        ) {
          syllable["hatoh"] = "ှ";
          syllable[component] = syllable[component].substring(0, 1);
        }
      } else if (component == "yayit") {
        if (syllable[component].charAt(1) == "ွ") syllable["wasway"] = "ွ";
        else if (syllable[component].charAt(1) == "ု") syllable["lVowel"] = "ု";
        else if (
          syllable[component].charAt(1) == "ိ" &&
          syllable[component].charAt(2) == "ု"
        ) {
          syllable["uVowel"] = "ိ";
          syllable["lVowel"] = "ု";
        } else
          this.debug.print("unhandled yayit ligature: " + syllable[component]);
        syllable[component] = syllable[component].substring(0, 1);
        ...
        if (
          syllable[component].charAt(3) == "ံ" ||
          (syllable[component].length > 4 &&
            syllable[component].charAt(4) == "ံ")
        )
          syllable["anusvara"] = "ံ";
        if (
          syllable[component].charAt(3) == "ိ" ||
          syllable[component].charAt(3) == "ီ"
        )
        ...
        this.debug.print(
          "unhandled ligature: " + component + " " + syllable[component],
        );
      }
    }
  }
  ...
  if (syllable["cons"] == "ဥ" && syllable["uVowel"] == "ီ") {
    ...
    if (
      ...
      (inputText.length > matchData.index + matchData[0].length &&
        (inputText
          .charAt(matchData.index + matchData[0].length)
      ...
      (inputText.length > matchData.index + matchData[0].length + 1 &&
        (inputText.charAt(matchData.index + matchData[0].length + 1) ==
          this.data["asat"]["်"] ||
          inputText
            .charAt(matchData.index + matchData[0].length + 1)
            .match(this.legacyStackPattern) ||
          inputText
            .charAt(matchData.index + matchData[0].length + 1)
            .match(this.legacyKinziPattern))) ||
      (inputText.length > matchData.index + matchData[0].length + 2 &&
        inputText.charAt(matchData.index + matchData[0].length + 2) ==
          this.data["asat"]["်"] &&
        inputText.charAt(matchData.index + matchData[0].length + 1) ==
          this.data["asat"]["့"])
    ) {
    ...
    syllable["cons"] == "၄" &&
    inputText.length >= matchData.index + matchData[0].length + 3
  ) {
    // check for lagaun
    if (
      inputText.substr(matchData.index + matchData[0].length, 3) ==
      this.data["cons"]["င"] +
        this.data["asat"]["်"] +
        this.data["visarga"]["း"]
    ) {
  ...
  for (var i = 0; i < outputOrder.length; i++) {
    if (syllable[outputOrder[i]]) outputText += syllable[outputOrder[i]];
  }
  return outputText;
};
 ...
 * @internal
 */
TlsMyanmarConverter.prototype.fromUnicodeMapper = function (
  inputText,
  matchData,
) {
    ...
    if (syllable[component])
      this.debug.print(
        "Legacy Syllable:" +
          ...
          syllable[component] +
          " / " +
          this.data[component][matchData[g]],
      );
  ...
  if (unicodeSyllable["kinzi"]) {
    if (unicodeSyllable["uVowel"]) {
      if (unicodeSyllable["anusvara"]) {
        var key =
          unicodeSyllable["kinzi"] +
          unicodeSyllable["uVowel"] +
          unicodeSyllable["anusvara"] +
          "_lig";
  ...
  if (unicodeSyllable["cons"] == "ဉ") {
    ...
    unicodeSyllable["cons"] == "၎င်း" &&
    this.data["cons"]["၎င်း"].length == 0
  ) {
    if (this.data["၎"].length)
      syllable["cons"] =
        this.data["cons"]["၎"] +
    ...
    if (
      unicodeSyllable["stack"] ||
      unicodeSyllable["yapin"] ||
      unicodeSyllable["wasway"] ||
      unicodeSyllable["hatoh"] ||
    ...
    if (
      ...
      unicodeSyllable["hatoh"] &&
      this.data["cons"][unicodeSyllable["cons"] + "_tall"].length
    ) {
  ...
  if (
    (unicodeSyllable["cons"] == "ခ" ||
      unicodeSyllable["cons"] == "ဂ" ||
      unicodeSyllable["cons"] == "င" ||
      unicodeSyllable["cons"] == "စ" ||
      unicodeSyllable["cons"] == "ဎ" ||
      unicodeSyllable["cons"] == "ဒ" ||
      unicodeSyllable["cons"] == "ဓ" ||
      unicodeSyllable["cons"] == "န" ||
      unicodeSyllable["cons"] == "ပ" ||
      unicodeSyllable["cons"] == "ဖ" ||
      unicodeSyllable["cons"] == "ဗ" ||
      unicodeSyllable["cons"] == "မ" ||
      unicodeSyllable["cons"] == "ဝ") &&
    unicodeSyllable["stack"] &&
    this.data["stack"][unicodeSyllable["stack"] + "_narrow"] &&
  ...
  if (
    unicodeSyllable["yapin"] &&
    (unicodeSyllable["wasway"] || unicodeSyllable["hatoh"])
  ) {
    if (this.data["yapin"]["ျ_alt"].length) {
      syllable["yapin"] = this.data["yapin"]["ျ_alt"];
    } // assume we have the ligatures
    else {
      var key =
        "ျ" +
        (unicodeSyllable["wasway"] ? "ွ" : "") +
        (unicodeSyllable["hatoh"] ? "ှ" : "") +
        "_lig";
  ...
  if (unicodeSyllable["yayit"]) {
    ...
    if (unicodeSyllable["wasway"]) {
      if (unicodeSyllable["hatoh"]) {
        if (this.data["wasway"]["ွှ_small"].length) {
          if (this.data["yayit"]["ြ" + upperVariant + widthVariant].length) {
            syllable["yayit"] =
              this.data["yayit"]["ြ" + upperVariant + widthVariant];
          } else {
            if (widthVariant == "_narrow") widthVariant = "";
          ...
          if (
            this.data["yayit"]["ြ_lower" + upperVariant + widthVariant].length
          )
            syllable["yayit"] =
              this.data["yayit"]["ြ_lower" + upperVariant + widthVariant];
          else syllable["yayit"] = this.data["yayit"]["ြ_lower" + widthVariant];
        }
      } else if (
        this.data["yayit"]["ြွ" + upperVariant + widthVariant].length
      ) {
        syllable["yayit"] =
          this.data["yayit"]["ြွ" + upperVariant + widthVariant];
        ...
        if (
          this.data["yayit"]["ြ" + "_lower" + upperVariant + widthVariant]
            .length
        )
          syllable["yayit"] =
            this.data["yayit"]["ြ" + "_lower" + upperVariant + widthVariant];
      ...
      unicodeSyllable["lVowel"] == "ု" &&
      this.data["yayit"]["ြု_wide"]
    ) {
      if (
        syllable["uVowel"] == this.data["uVowel"]["ိ"] &&
        this.data["yayit"]["ြို" + widthVariant]
      ) {
        ...
        if (this.data["yayit"]["ြု" + upperVariant + widthVariant].length)
          syllable["yayit"] =
            this.data["yayit"]["ြု" + upperVariant + widthVariant];
        else syllable["yayit"] = this.data["yayit"]["ြု" + widthVariant];
      }
  ...
  if (
    ...
    syllable["hatoh"] =
      this.data["hatoh"]["ှ" + unicodeSyllable["lVowel"] + "_lig"];
    delete syllable["lVowel"];
  }
  if (
    syllable["uVowel"] &&
    unicodeSyllable["uVowel"] == "ိ" &&
    syllable["anusvara"] &&
  ...
  if (
    syllable["lVowel"] &&
    (unicodeSyllable["yayit"] ||
      unicodeSyllable["yapin"] ||
      unicodeSyllable["wasway"] ||
      unicodeSyllable["hatoh"] ||
      unicodeSyllable["lig"] ||
      unicodeSyllable["stack"] ||
      unicodeSyllable["cons"] == "ဍ" ||
      unicodeSyllable["cons"] == "ဋ" ||
      unicodeSyllable["cons"] == "ဌ" ||
      unicodeSyllable["cons"] == "ဈ" ||
      unicodeSyllable["cons"] == "ဥ" ||
      unicodeSyllable["cons"] == "ဠ")
  ) {
    syllable["lVowel"] =
      this.data["lVowel"][unicodeSyllable["lVowel"] + "_tall"];
  }
  if (
    unicodeSyllable["aVowel"] &&
    unicodeSyllable["asat"] &&
    unicodeSyllable["aVowel"] == "ါ"
  ) {
    syllable["aVowel"] = this.data["aVowel"]["ါ်_lig"];
    delete syllable["asat"];
  }
  if (
    unicodeSyllable["lDot"] &&
    (unicodeSyllable["aVowel"] ||
      !(
        unicodeSyllable["yayit"] ||
        unicodeSyllable["lig"] ||
        ...
        unicodeSyllable["cons"] == "ဌ" ||
        unicodeSyllable["cons"] == "ဈ" ||
        unicodeSyllable["cons"] == "ရ"
      ))
  ) {
    if (unicodeSyllable["cons"] == "န")
      syllable["lDot"] = this.data["lDot"]["့_alt"];
    else syllable["lDot"] = this.data["lDot"]["့_left"];
  }
  if (
    unicodeSyllable["lDot"] &&
    !syllable["yayit"] &&
...
TlsMyanmarConverter.prototype.matchFrequency = function (inputText, isUnicode) {
  ...
  while (match) {
    var nonMatched = inputText.substring(pos, match.index);
    if (nonMatched.length) retValue.syllables.push(nonMatched);
  ...
  var freq = matchCharCount
    ? matchCharCount / (inputText.length - nonMyanmarCount)
    : 0;
  this.debug.print(
    "match uni=" +
      isUnicode +
      ...
      matchCharCount +
      " unmatched=" +
      (inputText.length - nonMyanmarCount - matchCharCount) +
      " length=" +
      inputText.length,
  );
  retValue.freq = freq;
...
TlsMyanmarConverter.prototype.getFontFamily = function (isUnicode) {
  return isUnicode
    ? "'Padauk','ThanLwin','Myanmar3','Parabaik'"
    : this.fontFamily;
};
...
function TlsMyanmarUtn11() {
  ...
  this.cons =
    "(က|ခ|ဂ|ဃ|င|စ|ဆ|ဇ|ဈ|ဉ|ည|ဋ|ဌ|ဍ|ဎ|ဏ|တ|ထ|ဒ|ဓ|န|ပ|ဖ|ဗ|ဘ|မ|ယ|ရ|လ|ဝ|သ|ဟ|ဠ|အ|ဣ|ဤ|ဥ|ဦ|ဧ|ဩ|ဪ|ဿ|၀|၁|၂|၃|၄|၅|၆|၇|၈|၉|၌|၍|၎|၏|ၐ|ၑ|ၒ|ၓ|ၔ|ၕ|ၚ|ၛ|ၜ|ၝ|ၡ|ၥ|ၦ|ၮ|ၯ|ၰ|ၵ|ၶ|ၷ|ၸ|ၹ|ၺ|ၻ|ၼ|ၽ|ၾ|ၿ|ႀ|ႁ|ႎ|႐|႑|႒|႓|႔|႕|႖|႗|႘|႙|႟|ꩠ|ꩡ|ꩢ|ꩣ|ꩤ|ꩥ|ꩦ|ꩧ|ꩨ|ꩩ|ꩪ|ꩫ|ꩬ|ꩭ|ꩮ|ꩯ|ꩱ|ꩲ|ꩳ|ꩴ|ꩵ|ꩶ|꩷|꩸|꩹|ꩺ)"; //3
  this.stack =
    "(\u1039(က|ခ|ဂ|ဃ|င|စ|ဆ|ဇ|ဈ|ဉ|ည|ဋ|ဌ|ဍ|ဎ|ဏ|တ|ထ|ဒ|ဓ|န|ပ|ဖ|ဗ|ဘ|မ|ယ|ရ|လ|ဝ|သ|ဟ|ဠ|အ|ၚ|ၛ|ၜ|ၝ)){0,2}"; //4
  this.asat = "(\u103A)?"; //6,11,22
...
TlsMyanmarUtn11.prototype.findSyllables = function (inputText) {
  ...
  while (match) {
    var nonMatched = inputText.substring(pos, match.index);
    if (nonMatched.length) syllables.push(nonMatched);
