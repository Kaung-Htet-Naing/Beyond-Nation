...
function TlsColor(color) {
  ...
  try {
    ...
    TlsDebug().print("TlsColor exception: " + e);
  }
...
TlsColor.prototype.inverse = function () {
  ...
  if (this.alpha < 1) inverted.alpha = 1 - this.alpha;
...
TlsFont.prototype.computedStyle = function (node) {
  if (typeof node.currentStyle != "undefined") return node.currentStyle;
...
TlsFont.prototype.nodeFontSize = function (node) {
  ...
  if (computedStyle) {
    ...
    else if (fontSizeText.indexOf("pt") > -1)
      // pt scaling at 96dpi/72
      fontSize =
        1.333 * Number(fontSizeText.substring(0, fontSizeText.indexOf("pt")));
    else if (fontSizeText.indexOf("em") > -1)
      fontSize =
        fontSize *
        Number(fontSizeText.substring(0, fontSizeText.indexOf("em")));
  }
  return fontSize;
};
...
function TlsTextRun(tlsFont, fontSize, text) {
  ...
  this.lineHeight =
    (tlsFont.font.data.ascent - tlsFont.font.data.descent) * this.scaling;
...
TlsTextRun.prototype.layoutText = function (tlsFont) {
  ...
  for (var i = 0; i < this.text.length; i++) {
    ...
    for (
      var j = i;
      j < this.text.length && j < i + tlsFont.font.maxContext;
      j++
    ) {
...
TlsTextRun.prototype.addGlyphs = function (startChar, endChar, glyphData) {
  ...
  for (i = 0; i < glyphData.length; i++) {
    ...
    if (i % 3 == 0) this.glyphLayout.push(value + endPos);
    else this.glyphLayout.push(value);
  }
  //	TlsDebug().print("addGlyphs" + startChar + "," + endChar + " " + this.glyphLayout + " at g " + glyphIndex + " " + glyphData);
  // update positions
  for (i = startChar; i <= endChar; i++) this.charToGlyph[i] = glyphIndex;
};
...
TlsTextRun.prototype.onmouseup = function (evt) {
  ...
  TlsDebug().print(
    this.text.substring(this.selection.first, this.selection.last + 1),
  );
  return true;
};
...
TlsTextRun.prototype.findCharPosition = function (evt) {
  ...
  for (; startChar < this.charToGlyph.length; startChar++) {
    endChar = startChar;
    while (
      endChar + 1 < this.charToGlyph.length &&
  ...
  TlsDebug()
    ...
    .print(
      "glyph: " +
        ...
        scaledDx +
        " " +
        this.text.substring(startChar, endChar + 1),
    );
...
TlsFont.prototype.appendText = function (
  parent,
  fontName,
  size,
  text,
...
function TlsNodePosition(node) {
  ...
  while (parentElem) {
    ...
    if (hasOffsetParent) parentElem = parentElem.offsetParent;
